<script>
	...
	const colunas = ['Ideia', 'Pendente', 'Em Processo', 'Finalizado'];

	let draggedId = null;
	let editingId = null;

	function handleDragStart(event, projeto) {
	...
	function toggleEdit(projeto) {
		editingId = editingId === projeto.id ? null : projeto.id;
	}

	function updateProjectField(projetoId, field, value) {
		projetos.update((items) => {
...
<div class="container">
	...
	<div class="kanban">
		{#each colunas as status}
			<div class="coluna" on:dragover|preventDefault on:drop={(event) => handleDrop(event, status)}>
				<h2>{status}</h2>
